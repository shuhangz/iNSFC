# iNSFC - 2022 年国家自然基金 LaTeX 模板
***
**2022版说明：** 更新为2022国家自然青年基金版本，并加入一些其他改动：
1. 参考文献使用Biblatex，author-year模式，但文后每个参考文献加入了序号。
2. 使用`url`宏包，并且加入url断行控制。
3. 字体使用`Simsun`宋体，包含生僻字。
  
**编译方法：** xelatex->biber->xelatex*2
***

**说明：** 此模板是在[戴一冕](https://github.com/YimianDai/iNSFC)的2017年国家自然科学基金LaTeX模板的基础上，针对2020年国家自然科学基金的模板修改而来。改动如下：

+ 将原模板封装为iNSFC.cls类模板，调用更加方便
+ 原模板定义的\NsfcSection命令需要手动指定编号，改为自动编号
+ 2020年模板的“报告正文”四个字由2号字变为加粗楷体三号，做了改动
+ 将正文之前的话变为`\qianyan`，使源码文件看起来更简洁
+ 定义`bibenv`环境，隐藏排版细节

有问题或改动不合理之处，欢迎各路大神指教~
...
+ **文字：**
  + **正文标题：**字体是 加粗的楷体三号，段后间距 0.5 行，行距为固定值 22 磅；
...
另一个模板是2017年1月8号，南开大学的[程明明教授](http://mmcheng.net)，曾在 CTeX 论坛上给出过一个[国家自然科学基金的 LaTeX 模版](http://www.latexstudio.net/archives/9308)。受程明明教授先驱工作的启发，我们也做了一点微小的工作。
...
感谢[戴一冕](https://github.com/YimianDai/iNSFC)的iNSFC模板~